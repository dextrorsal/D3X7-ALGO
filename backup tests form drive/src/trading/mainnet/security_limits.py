...
class SecurityLimits:
    ...
    def validate_trade_size(self, market: str, size: float, quote_price: Optional[float] = None) -> bool:
        """
        ...
        if market.startswith("SOL"):
            if size > self.limits["max_trade_size"]:
                logger.warning(f"Trade size {size} SOL exceeds maximum allowed {self.limits['max_trade_size']} SOL")
                return False
        # For non-SOL markets with a quote price, calculate equivalent SOL value
        elif quote_price is not None:
            # This is a simplified conversion that assumes the quote price is in USDC
            # For real implementation, you'd need to convert to SOL value based on current SOL price
            ...
            logger.warning(f"Cannot validate non-SOL market {market} without a quote price")
            return False
            
        return True
    
    def validate_position_size(self, market: str, new_size: float, current_size: float = 0) -> bool:
        """
    ...
    def validate_leverage(self, market: str, leverage: float) -> bool:
        ...
        max_leverage = self.limits["max_leverage"].get(
            market, 
            self.limits["max_leverage"]["default"]
        )
    ...
    def update_daily_volume(self, trade_size: float, quote_price: Optional[float] = None) -> bool:
        """
    ...
    def check_emergency_shutdown(self, 
                               ...
                               current_drawdown: Optional[float] = None) -> bool:
        """
        ...
        if (current_loss_pct is not None and 
            current_loss_pct > triggers["loss_threshold_pct"]):
            logger.error(
        ...
        # Check volume spike
        if (current_volume is not None and 
            Decimal(str(current_volume)) > self.daily_volume * Decimal(str(triggers["volume_spike_multiplier"]))):
            logger.error(
        ...
        # Check drawdown
        if (current_drawdown is not None and 
            current_drawdown > triggers["max_drawdown_pct"]):
            logger.error(
