...
class SOLSpotStrategy(SegmentedStrategy):
    ...
    def __init__(self, config: Dict):
        ...
        volatile_indicators = [
            ...
            LorentzianIndicator({    # Good for classification in volatile periods
                "general_settings": {
                    "neighbors_count": 8,
                    "max_bars_back": 2000
                },
    ...
    def _detect_market_regime(self, df: pd.DataFrame) -> str:
        ...
        if current_bandwidth > 0.06 or vol_spike:
            logger.debug(f"Detected volatile regime (bandwidth: {current_bandwidth:.4f}, vol_spike: {vol_spike})")
            return "volatile"
        # 2. Check for strong trending
        elif directional_strength > self.trend_threshold:
            logger.debug(f"Detected trending regime (directional_strength: {directional_strength:.4f})")
            return "trend"
        # 3. Default to ranging if not volatile or trending
        else:
            logger.debug(f"Detected ranging regime (directional_strength: {directional_strength:.4f})")
            return "range"
    
    def _adjust_weights(self, df: pd.DataFrame, regime: str, indicators: List, weights: List[float]) -> List[float]:
        """
        ...
        future_return = df['close'].pct_change().shift(-1).iloc[-21]  # Next bar's return
        ...
        for i, indicator in enumerate(indicators):
            ...
            signal_correct = (last_signal > 0 and future_return > 0) or (last_signal < 0 and future_return < 0)
            ...
            if signal_correct:
                ...
                updated_weights[i] *= (1 - adjustment * 0.5)  # Penalize less for wrong signals
        
        # Normalize weights to maintain scale
        total = sum(updated_weights)
        if total > 0:
            updated_weights = [w/total * len(updated_weights) for w in updated_weights]
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        if len(df) > 20:
            ...
            if len(volatility) >= len(final_signals):
                extreme_vol = volatility.iloc[-len(final_signals):] > 0.05  # 5% daily std dev is extreme
        ...
        if len(df) > 1:
            ...
            if len(future_returns) >= len(final_signals):
                ...
                matched_signals = final_signals.iloc[:-1]  # Exclude last signal (no future return yet)
                
                # Calculate if signals predicted direction correctly
                signal_quality = (matched_signals * matched_returns).mean()
                self.performance_history['regime_performance'][regime].append(signal_quality)
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        for regime, performances in self.performance_history['regime_performance'].items():
            if performances:
                metrics[f'{regime}_avg_quality'] = np.mean(performances)
                metrics[f'{regime}_samples'] = len(performances)
        
        # Overall signal quality
        all_qualities = [item['quality'] for item in self.performance_history['signal_quality'] 
