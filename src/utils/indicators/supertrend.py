...
class Supertrend:
    ...
    def calculate(self, df):
        ...
        for i in range(self.atr_period, len(self.df)):
            ...
            if close.iloc[current] > self.final_upperband.iloc[prev]:
                self.final_upperband.iloc[current] = max(upperband.iloc[current], self.final_upperband.iloc[prev])
            else:
                self.final_upperband.iloc[current] = upperband.iloc[current]
                
            if close.iloc[current] < self.final_lowerband.iloc[prev]:
                self.final_lowerband.iloc[current] = min(lowerband.iloc[current], self.final_lowerband.iloc[prev])
            else:
            ...
            if self.df['uptrend'].iloc[current] and close.iloc[current] <= self.final_upperband.iloc[current]:
                self.df.loc[self.df.index[current], 'in_uptrend'] = False
            elif not self.df['uptrend'].iloc[current] and close.iloc[current] >= self.final_lowerband.iloc[current]:
                self.df.loc[self.df.index[current], 'in_uptrend'] = True
            else:
                self.df.loc[self.df.index[current], 'in_uptrend'] = self.df['in_uptrend'].iloc[prev]
                
            # Set Supertrend value
            if self.df['in_uptrend'].iloc[current]:
                self.df.loc[self.df.index[current], 'supertrend'] = self.final_lowerband.iloc[current]
            else:
                self.df.loc[self.df.index[current], 'supertrend'] = self.final_upperband.iloc[current]
    ...
    def get_trend_changes(self):
        ...
        if not trend_changes.empty:
            ...
            trend_changes['signal'] = trend_changes['direction'].map({True: 'Buy', False: 'Sell'})
...
class SupertrendIndicator(BaseIndicator):
    ...
    def __init__(self, config_path='config/indicator_settings.json'):
        config = Config()
        supertrend_config = config.get('indicators', {}).get('supertrend', {})
        
        self.atr_period = supertrend_config.get('atr_period', 14)
        self.multiplier = supertrend_config.get('multiplier', 3.0)
        self.filter_signals_by = supertrend_config.get('filter_signals_by', 'None')
        self.holding_period = supertrend_config.get('holding_period', 5)
        self.debug = False
