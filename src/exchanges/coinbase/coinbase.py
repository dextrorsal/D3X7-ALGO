...
class CoinbaseHandler(BaseExchangeHandler):
        ...
        try:
            while start_time < end_time:
                ...
                if isinstance(response_data, dict):
                    candle_list = response_data.get('candles', []) or response_data.get('data', [])
    ...
    async def fetch_live_candles(
        ...
        try:
            ...
            current_time = datetime.strptime(trade_data['time'], "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
