...
class DriftClient:
    ...
    def _get_rpc_url(self) -> str:
        ...
        if self.network == 'mainnet':
            # Try Helius first, then other mainnet endpoints
            rpc_urls = [
        ...
        return "https://api.mainnet-beta.solana.com" if self.network == 'mainnet' else "https://api.devnet.solana.com"
    
    async def _connect_with_retry(self, max_retries: int = 5, initial_delay: float = 1.0) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Connection attempt {attempt + 1} failed with {str(e)}, retrying in {delay}s")
                    await asyncio.sleep(delay)
                    delay *= 2  # Exponential backoff
                else:
                    logger.error(f"Failed to connect after {max_retries} attempts")
                    raise ExchangeError(f"Failed to connect to Drift: {str(last_error)}")
