...
class MockBinanceHandler(BaseExchangeHandler):
    """Mock handler for Binance exchange data."""
    ...
    def validate_market(self, market: str) -> bool:
        ...
        try:
            ...
            return (
                binance_market in self._available_markets or
                any(self._convert_market_symbol(m) == binance_market for m in self.config.markets)
            )
        except ValueError:
            return False
    ...
    def _convert_market_symbol(self, market: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Symbol mapper failed to convert {market}, falling back to basic conversion")
    ...
    async def fetch_historical_candles(
        ...
        while current_time <= time_range.end:
            ...
            current_time = datetime.fromtimestamp(current_time.timestamp() + seconds, tz=timezone.utc)
    ...
    async def get_markets(self) -> List[str]:
        """Get mock available markets in Binance's native format."""
            ...
            'symbols': [
                {
                    ...
                    'status': 'TRADING',
                    'baseAsset': self.symbol_mapper.from_exchange_symbol("binance", market).split('-')[0],
                    'quoteAsset': 'USDT',
