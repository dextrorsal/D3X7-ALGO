...
from src.trading.drift.drift_qt_gui import DriftGUI
from src.trading.drift.drift_position_monitor import DriftPositionMonitor
...
class DriftCLI(BaseCLI):
    ...
    def add_arguments(self, parser: _SubParsersAction) -> None:
        ...
        gui_parser.add_argument(
            "--wallet", required=True, help="Wallet to use for trading"
        )
        gui_parser.add_argument(
            "--dark-mode", action="store_true", help="Use dark mode theme"
        )
        ...
        balance_parser = subparsers.add_parser(
            "balance", help="Check wallet balance and positions"
        )
        balance_parser.add_argument("--wallet", required=True, help="Wallet name")
        balance_parser.add_argument(
            "--subaccount", type=int, default=0, help="Subaccount ID to check"
        )

        # Subaccounts command
        subaccounts_parser = subparsers.add_parser(
            "subaccounts", help="List subaccounts"
        )
        subaccounts_parser.add_argument(
            "--wallet", help="Optional wallet name (lists all if not specified)"
        )

        # Position management
        position_parser = subparsers.add_parser("position", help="Manage positions")
        position_parser.add_argument(
            "--market", required=True, help="Market symbol (e.g., SOL-PERP)"
        )
        position_parser.add_argument(
            "--size", type=float, required=True, help="Position size"
        )
        position_parser.add_argument(
            "--side", choices=["long", "short"], required=True, help="Position side"
        )
        position_parser.add_argument(
            "--leverage", type=float, default=1.0, help="Position leverage"
        )
        position_parser.add_argument(
            "--reduce-only", action="store_true", help="Make this a reduce-only order"
        )

        # Close position command
        close_parser = subparsers.add_parser("close", help="Close positions")
        close_parser.add_argument(
            "--market", required=True, help="Market to close position for"
        )
        close_parser.add_argument(
            "--size",
            type=float,
        ...
        health_parser.add_argument(
            "--market", required=True, help="Market to check health for"
        )
    ...
    async def _handle_gui(self, args: Any) -> None:
        ...
        try:
            ...
            # Initialize GUI
            self.gui = DriftGUI(
                wallet=wallet, config=self.config, dark_mode=args.dark_mode
            )
        ...
        for wallet_name in wallets:
            ...
            if wallet:
                ...
                table.add_row(
                    ...
                    and self.wallet_manager.current_wallet.name == wallet_name
                    else "",
                )
    ...
    async def _handle_subaccounts(self, args: Any) -> None:
        ...
        try:
            wallets_to_check = (
                [args.wallet] if args.wallet else self.wallet_manager.list_wallets()
            )
            ...
            for wallet_name in wallets_to_check:
                ...
                if not subaccounts:
                    console.print(
                        f"[yellow]No subaccounts found for wallet {wallet_name}[/yellow]"
                    )
                    continue
                ...
                for subaccount in subaccounts:
                    table.add_row(
                        str(subaccount.id),
                        subaccount.name or "Main"
                        if subaccount.id == 0
                        else f"Subaccount {subaccount.id}",
    ...
    async def _handle_close(self, args: Any) -> None:
        ...
        try:
            result = await self.adapter.close_position(
                market=args.market, size=args.size
            )
