...
if DRIFT_NETWORK != "mainnet-beta":
    logger.warning(
        f"DRIFT_NETWORK set to {DRIFT_NETWORK}, should be 'mainnet-beta' for mainnet trading"
    )
...
async def execute_jupiter_swap(
    ...
    try:
        ...
        result = await adapter.execute_swap(market=market, input_amount=input_amount)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Execute trades on mainnet with security limits"
    )
    parser.add_argument(
        "--market", required=True, help="Market symbol (e.g., SOL-PERP or SOL-USDC)"
    )
    parser.add_argument(
        "--size", type=float, required=True, help="Trade size or input amount"
    )
    parser.add_argument(
        "--side",
        default="buy",
        choices=["buy", "sell"],
        help="Trade side (for perp markets)",
    )
    ...
    if is_perp:
        result = await execute_drift_trade(
            market=args.market, size=args.size, side=args.side, wallet_name=args.wallet
        )
    else:
        result = await execute_jupiter_swap(
            market=args.market, input_amount=args.size, wallet_name=args.wallet
        )
