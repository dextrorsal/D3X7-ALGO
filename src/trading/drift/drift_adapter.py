...
class DriftAdapter:
    ...
    def __init__(self, wallet_manager, network="devnet"):
    ...
    async def connect(self) -> bool:
        ...
        try:
            # Initialize authentication with WalletManager
            self.auth = DriftAuth(
                wallet_manager=self.wallet_manager, network=self.network
            )
            # Authenticate
            await self.auth.authenticate()
            # Initialize Drift client with authenticated wallet
            self.client = DriftClient(self.auth.get_wallet(), network=self.network)
    ...
    async def place_order(
        self, market: str, side: str, size: float, price: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def get_position(self, market: str) -> Dict[str, Any]:
        ...
        try:
            ...
            leverage = (
                position.leverage / MARGIN_PRECISION
