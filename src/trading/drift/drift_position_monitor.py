...
Real-time position monitoring system for Drift market making,
with advanced analytics and risk management features.
"""
...
class DriftPositionMonitor:
    ...
    def __init__(
        ...
        self.account_manager = DriftAdapter(self.wallet_name)
    ...
    async def initialize(self):
        ...
        try:
            ...
            config = self.drift_wallet_manager.get_subaccount_config(
                self.wallet_name, self.subaccount_id
            )
            if not config:
                raise Exception(
                    f"Subaccount {self.subaccount_id} not found for wallet {self.wallet_name}"
                )
    ...
    async def update_market_data(self):
        ...
        try:
            ...
            for market in markets:
                ...
                self.mm_metrics["inventory_skew"][market_index] = (
                    self._calculate_inventory_skew(market)
                )
                self.mm_metrics["order_imbalance"][market_index] = (
                    self._calculate_order_imbalance(market)
                )
                self.mm_metrics["market_impact"][market_index] = (
                    self._calculate_market_impact(market)
                )
                self.mm_metrics["funding_rates"][market_index] = (
                    market.amm.last_funding_rate / 1e6
                )
                self.mm_metrics["volume_24h"][market_index] = (
                    market.amm.volume24h / QUOTE_PRECISION
                )
                self.mm_metrics["quote_depth"][market_index] = (
                    self._calculate_quote_depth(market)
                )
    ...
    def _calculate_order_imbalance(self, market) -> float:
        ...
        try:
            ...
            return (
                ((bids_value - asks_value) / total_value * 100)
                if total_value != 0
                else 0
            )
        except:
    ...
    async def update_positions(self):
        ...
        try:
            ...
            # Get unrealized PnL
            upnl = await self.account_manager.drift_user.get_unrealized_pnl(
                with_funding=True
            )
            ...
            # Get free collateral
            free_collateral = (
                await self.account_manager.drift_user.get_free_collateral()
            )
        ...
        try:
            for pos in self.positions["positions"]:
                ...
                if size != 0:
                    ...
                    liquidation_price = (
                        entry_price * (1 - margin_ratio)
                        if size > 0
                        else entry_price * (1 + margin_ratio)
                    )
                    self.risk_metrics["liquidation_prices"][market_index] = (
                        liquidation_price
                    )
                    ...
                    total_position_value = sum(
                        abs(p.base_asset_amount / BASE_PRECISION * p.entry_price / 1e6)
                    ...
                    self.risk_metrics["concentration_risk"][market_index] = (
                        concentration
                    )
    ...
    async def update_performance_metrics(self):
        ...
        try:
            ...
            self.performance_metrics["realized_pnl_24h"] = Decimal(
                str(self.positions.get("upnl", 0) / 1e6)
            )

            # Calculate ROI
            initial_collateral = Decimal("1000")  # Example value
            current_collateral = Decimal(str(self.positions["free_collateral"] / 1e6))
            self.performance_metrics["roi_24h"] = (
                (current_collateral - initial_collateral) / initial_collateral * 100
            )
    ...
    def create_performance_table(self) -> Table:
        ...
        metrics = [
            (
                "24h Realized PnL",
                f"${float(self.performance_metrics['realized_pnl_24h']):,.2f}",
            ),
    ...
    def print_status(self):
        ...
        print(
            f"Last Update: {Colors.GREEN}{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.RESET}\n"
        )
