...
class DriftGUI(QMainWindow):
    def __init__(self):
        ...
        self.drift_manager = DriftWalletManager()  # For Drift-specific operations
        ...
        async def setup_async():
            try:
                ...
                if not self.account_manager:
                    ...
                    self.account_manager = DriftAccountManager(wallet_name)
    ...
    def on_wallet_changed(self, index_or_name):
        ...
        try:
            ...
            if index >= 0:
                ...
                print(f"Initializing DriftAccountManager for {wallet_name}...")
                self.account_manager = DriftAccountManager(wallet_name)
    ...
    def setup_positions_tab(self):
        ...
        self.positions_table.setHorizontalHeaderLabels(["Market", "Size", "Entry Price", "Current Price", "PnL", "Leverage"])
    ...
    def setup_orders_tab(self):
        ...
        self.orders_table.setHorizontalHeaderLabels(["Market", "Side", "Size", "Price", "Status"])
        ...
        try:
            ...
            try:
                ...
                for spot in spot_positions:
                    ...
                    token_name = "USDC" if market_index == 0 else self.spot_markets.get(market_index, f"Unknown-{market_index}")
                    
                    # Convert token amounts from lots to normal format
                    token_amount = spot.scaled_balance
                    print(f"Spot position: {token_name}, market_index: {market_index}, balance: {token_amount}")
            ...
            try:
                ...
                for pos in perp_positions:
                    ...
                    market_name = self.perp_markets.get(market_index, f"Unknown-{market_index}")
                    ...
                    market_price = self.account_manager.drift_client.get_perp_market_price(market_index) / 1e6
                ...
                for spot in spot_positions:
                    if spot.market_index == 0 or spot.scaled_balance == 0:  # Skip USDC or zero balances
                        continue
                    
                    market_index = spot.market_index
                    market_name = self.spot_markets.get(market_index, f"Unknown-{market_index}")
                    
                    # Get token price relative to USDC
                    token_price = self.account_manager.drift_client.get_spot_market_price(market_index) / 1e6
                    ...
                    print(f"Added spot position: {market_name}, size: {spot.scaled_balance}, price: {token_price}")
                ...
                for i, pos in enumerate(positions_to_display):
                    ...
                    # Size
                    self.positions_table.setItem(i, 1, QTableWidgetItem(f"{pos['size']:.4f}"))
                    
                    # Entry Price
                    entry_price_str = f"${pos['entry_price']:.2f}" if pos['entry_price'] > 0 else "N/A"
                    self.positions_table.setItem(i, 2, QTableWidgetItem(entry_price_str))
                    
                    # Current Price
                    self.positions_table.setItem(i, 3, QTableWidgetItem(f"${pos['current_price']:.2f}"))
                    ...
                    # Leverage
                    self.positions_table.setItem(i, 5, QTableWidgetItem(f"{pos['leverage']:.2f}x"))
                
                # Update the balance label with total PnL and leverage
                total_balance = balances.get("USDC", 0)
                self.balance_label.setText(f"Balance: ${total_balance:.2f} USDC | PnL: ${total_pnl:.2f} | Avg Leverage: {avg_leverage:.2f}x")
            ...
            try:
                ...
                active_orders = [order for order in orders if order.status <= 1]  # 0=open, 1=filled
                ...
                for i, order in enumerate(active_orders):
                    ...
                    if market_type == "PERP":
                        market_name = self.perp_markets.get(order.market_index, f"Unknown-{order.market_index}")
                    else:
                        market_name = self.spot_markets.get(order.market_index, f"Unknown-{order.market_index}")
                    
                    self.orders_table.setItem(i, 0, QTableWidgetItem(f"{market_name} ({market_type})"))
    ...
    def on_long_clicked(self):
        ...
        if self.account_manager:
            try:
                ...
                market_index = self.perp_market_indices.get(market, 0)  # Default to SOL-PERP (0) if not found
                print(f"Opening LONG position on {market} (index: {market_index})")
                
                # TODO: Add position size dialog
                size = 1.0  # For testing
                self.run_async(self.account_manager.place_perp_order(
            ...
            try:
                ...
                for pos in positions:
                    if pos.market_index == market_index:
                        ...
                        print(f"Closing position: {size} in direction {direction} (1=short, 0=long)")
                        self.run_async(self.account_manager.place_perp_order(
                            market_index=market_index,
            ...
            if self.account_manager and hasattr(self.account_manager, 'drift_client'):
                try:
                    ...
                    if hasattr(self.account_manager.drift_client, 'unsubscribe'):
                        loop.run_until_complete(self.account_manager.drift_client.unsubscribe())
    ...
    def setup_layout(self):
        ...
        self.wallet_combo.currentIndexChanged.connect(self.on_wallet_changed)  # Connect to index changed signal
    ...
    def setup_ui(self):
        ...
        print("Setting up UI components - no additional setup needed as it's handled in tabs")
        pass
