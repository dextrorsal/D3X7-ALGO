...
from src.exchanges.binance.binance_mock import (
    MockBinanceHandler,
)  # Add mock Binance handler
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "timeout: mark test to timeout after X seconds")
    config.addinivalue_line("markers", "slow: mark test as slow to run")
    config.addinivalue_line("markers", "integration: mark test as integration test")
    config.addinivalue_line("markers", "exchange: mark test as exchange-specific")
...
async def binance_handler(mock_config):
    ...
    handler = MockBinanceHandler(mock_config)
...
def binance_config():
    ...
    return ExchangeConfig(
        name="binance",
        credentials=ExchangeCredentials(
            api_key="test_key", api_secret="test_secret", additional_params={}
        ),
        rate_limit=10,
...
class TestCriticalExchangeFunctionality:
    ...
    async def test_historical_data_fetching(
        self, mock_config, time_range, test_markets
    ):
        """Test historical data fetching."""
        try:
            async with MockBinanceHandler(mock_config) as handler:
                # Enable test mode
                handler._is_test_mode = True
                handler._setup_test_mode()
