...
class DriftTools:
    def __init__(self, rpc_url_or_adapter: Union[str, DriftAdapter, None] = None):
        if isinstance(rpc_url_or_adapter, DriftAdapter):
            ...
            self.adapter = None
            self.rpc_url = rpc_url_or_adapter or os.environ.get("HELIUS_RPC_ENDPOINT", "https://api.mainnet-beta.solana.com")
        self.client = None
        logger.info(f"Initialized DriftTools with {'adapter' if self.adapter else 'RPC URL: ' + str(self.rpc_url)}")
    ...
    async def connect(self) -> None:
        ...
        try:
            if self.adapter:
                ...
                network = "devnet" if self.rpc_url and "devnet" in self.rpc_url else "mainnet"
    ...
    async def get_markets(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"DriftTools: perp_markets_result type: {type(perp_markets_result)}")
            ...
            if asyncio.iscoroutine(perp_markets_result):
                ...
                logger.info("DriftTools: using perp_markets_result directly (not a coroutine)")
                perp_markets = perp_markets_result

            logger.info(f"DriftTools: Got {len(perp_markets) if perp_markets else 0} perp markets")

            # Debug the first market structure
            if perp_markets and len(perp_markets) > 0:
                perp_market_sample = perp_markets[0]
                logger.info(f"DriftTools: Sample perp market attrs: {dir(perp_market_sample)}")
                logger.info(f"DriftTools: Sample perp market name type: {type(perp_market_sample.name)}")
                # Check if name is a ListContainer
                if hasattr(perp_market_sample.name, '__iter__') and not isinstance(perp_market_sample.name, (str, bytes)):
                    logger.info(f"DriftTools: Name is iterable, converting to string: {list(perp_market_sample.name)}")
            
            # Debug AMM structure
            if hasattr(perp_market_sample, 'amm'):
                logger.info(f"DriftTools: AMM attributes: {dir(perp_market_sample.amm)}")

            for i, perp_market in enumerate(perp_markets):
                status_str = str(perp_market.status)
                logger.info(f"DriftTools: Processing perp market {i}, status: {status_str}")
                ...
                if "Initialized" not in status_str:
                    ...
                    if hasattr(perp_market.name, '__iter__') and not isinstance(perp_market.name, (str, bytes)):
                    ...
                    price = None
                    if hasattr(perp_market.amm, 'oracle_price_data') and hasattr(perp_market.amm.oracle_price_data, 'price'):
                        price = perp_market.amm.oracle_price_data.price / QUOTE_PRECISION
                    elif hasattr(perp_market.amm, 'oracle') and hasattr(perp_market.amm.oracle, 'price'):
                        ...
                        logger.warning(f"DriftTools: Could not find price for market {market_symbol}, using default")
                    ...
                    logger.info(f"DriftTools: Skipped perp market with status {status_str}")

            logger.info("DriftTools: Attempting to get spot market accounts...")
            # Check if the method returns a coroutine or directly a list
            spot_markets_result = self.client.get_spot_market_accounts()
            logger.info(f"DriftTools: spot_markets_result type: {type(spot_markets_result)}")
            ...
            if asyncio.iscoroutine(spot_markets_result):
                ...
                logger.info("DriftTools: using spot_markets_result directly (not a coroutine)")
                spot_markets = spot_markets_result

            logger.info(f"DriftTools: Got {len(spot_markets) if spot_markets else 0} spot markets")
            ...
            if spot_markets and len(spot_markets) > 0:
                ...
                logger.info(f"DriftTools: Sample spot market attrs: {dir(spot_market_sample)}")
                # Debug spot market name
                if hasattr(spot_market_sample, 'name'):
                    logger.info(f"DriftTools: Sample spot market name type: {type(spot_market_sample.name)}")
            
            for i, spot_market in enumerate(spot_markets):
                status_str = str(spot_market.status)
                logger.info(f"DriftTools: Processing spot market {i}, status: {status_str}")
                
                # Check if the market is not initialized
                if "Initialized" not in status_str:
                    # Handle ListContainer name
                    if hasattr(spot_market.name, '__iter__') and not isinstance(spot_market.name, (str, bytes)):
                        # Convert ListContainer to string by joining the bytes
                    ...
                    # Get price safely
                    price = None
                    if hasattr(spot_market, 'oracle_price') and spot_market.oracle_price:
                        price = spot_market.oracle_price / QUOTE_PRECISION
                    elif hasattr(spot_market, 'oracle') and hasattr(spot_market.oracle, 'price'):
                    ...
                    logger.info(f"DriftTools: Skipped spot market with status {status_str}")
    ...
    async def get_prices(self, markets: Optional[List[Dict[str, Any]]] = None) -> Dict[str, float]:
        if markets is None:
