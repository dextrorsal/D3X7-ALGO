...
def security_config():
    ...
    return {
        "max_swap_size": {"SOL": 10.0, "USDC": 1000.0},
...
class TestMainnetSwap:
    ...
    async def test_jupiter_swap_execution(self, jupiter_adapter, security_config):
        ...
        result = await self.execute_test_swap(
            market="SOL-USDC", input_amount=1.0, security_config=security_config
        )
    ...
    async def test_swap_rejection_invalid_market(
        self, jupiter_adapter, security_config
    ):
        """Test swap rejection for invalid market."""
        result = await self.execute_test_swap(
            market="INVALID-USDC", input_amount=1.0, security_config=security_config
        )
    ...
    async def execute_test_swap(
        ...
        supported_markets = ["SOL-USDC", "BTC-USDC", "ETH-USDC"]
