...
def plot_signals(ax, df, signal_col, label):
    ...
    # Plot markers
    ax.plot(buys.index, buys["close"], "^", color="green", markersize=8, label=f"Buy ({len(buys)})")
    ax.plot(sells.index, sells["close"], "v", color="red", markersize=8, label=f"Sell ({len(sells)})")
...
def test_indicators():
    ...
    for i, (signal_col, label) in enumerate([
        ...
        # Plot markers
        plt.plot(buys.index, buys["close"], "^", color="green", markersize=8, label=f"Buy ({len(buys)})")
        plt.plot(sells.index, sells["close"], "v", color="red", markersize=8, label=f"Sell ({len(sells)})")
        ...
        logging.info(f"Saved {label} signals plot to {output_dir}/{label.lower()}_signals.png")
    ...
    with open(f"{output_dir}/indicator_analysis.txt", "w") as f:
        ...
        for label, analysis in [
            ...
            f.write(f"Buy signals: {analysis['buy_count']} ({analysis['buy_pct']:.2f}%)\n")
            f.write(f"Sell signals: {analysis['sell_count']} ({analysis['sell_pct']:.2f}%)\n")
            f.write(f"Neutral signals: {analysis['neutral_count']} ({analysis['neutral_pct']:.2f}%)\n")
            f.write(f"Average buy signal duration: {analysis['avg_buy_duration']:.2f} periods\n")
            f.write(f"Average sell signal duration: {analysis['avg_sell_duration']:.2f} periods\n")
            f.write(f"Average neutral signal duration: {analysis['avg_neutral_duration']:.2f} periods\n\n")
    
    logging.info(f"Saved indicator analysis summary to {output_dir}/indicator_analysis.txt")
    
    # Verify that all indicators are generating signals
    assert df["Supertrend_Signal"].nunique() > 1, "Supertrend indicator is not generating different signals"
    assert df["KNN_Signal"].nunique() > 1, "KNN indicator is not generating different signals"
    assert df["Logistic_Signal"].nunique() > 1, "Logistic indicator is not generating different signals"
    assert df["Lorentzian_Signal"].nunique() > 1, "Lorentzian indicator is not generating different signals"
    
    # Verify that all indicators have both buy and sell signals
    assert (df["Supertrend_Signal"] == 1).any(), "Supertrend indicator is not generating buy signals"
    assert (df["Supertrend_Signal"] == -1).any(), "Supertrend indicator is not generating sell signals"
    assert (df["KNN_Signal"] == 1).any(), "KNN indicator is not generating buy signals"
    assert (df["KNN_Signal"] == -1).any(), "KNN indicator is not generating sell signals"
    assert (df["Logistic_Signal"] == 1).any(), "Logistic indicator is not generating buy signals"
    assert (df["Logistic_Signal"] == -1).any(), "Logistic indicator is not generating sell signals"
    assert (df["Lorentzian_Signal"] == 1).any(), "Lorentzian indicator is not generating buy signals"
    assert (df["Lorentzian_Signal"] == -1).any(), "Lorentzian indicator is not generating sell signals"
...
def test_indicators_with_real_data():
    ...
    logging.info(f"Fetching {symbol} data with {timeframe} timeframe from Yahoo Finance...")
    ...
    logging.info(f"Saved indicator signals plot to {output_dir}/indicator_signals_{symbol}_{timeframe}.png")
    ...
    for i, (signal_col, label) in enumerate([
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(plot_df.index, plot_df["close"], label="Close", color="black", alpha=0.7)
        ...
        logging.info(f"Saved {label} signals plot to {output_dir}/{label.lower()}_{symbol}_{timeframe}.png")
    
    # Save the signal data to a CSV file for further analysis
    df.to_csv(f"{output_dir}/indicator_signals_{symbol}_{timeframe}.csv")
    logging.info(f"Saved indicator signals data to {output_dir}/indicator_signals_{symbol}_{timeframe}.csv")
    ...
    logging.info(f"Saved indicator analysis summary to {output_dir}/indicator_analysis_{symbol}_{timeframe}.txt")
