...
from src.utils.indicators.logistic_regression import LogisticRegressionIndicator
...
async def test_fetch_and_generate_signals(binance_handler, time_range):
    ...
    logger.info(f"Fetching {market} data for the last 5 days with {resolution} resolution...")
    ...
    for i, (name, indicator) in enumerate(indicators.items(), 2):
        ...
        try:
            ...
            logger.info(f"{name} signal counts: Buy={buy_count}, Sell={sell_count}, Neutral={neutral_count}")
            ...
            if not buy_indices.empty:
                ...
                plt.scatter(buy_indices, buy_prices, color='green', marker='^', s=50, label='Buy')
            ...
            if not sell_indices.empty:
                ...
                plt.scatter(sell_indices, sell_prices, color='red', marker='v', s=50, label='Sell')
